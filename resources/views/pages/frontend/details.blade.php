...
<section class="container mx-auto">
  <div class="flex flex-wrap my-4 md:my-12">
    <div class="w-full md:hidden px-4">
      ...
      <p class="text-xl py-3">IDR {{ number_format($products->price) }}</p>
    </div>
    <div class="flex-1">
      <div class="slider">
        <div class="thumbnail">
    ...
    <div class="flex-1 px-4 md:p-6 ">
      <div class="hidden md:block">
        <h2 class="text-5xl font-semibold my-2">{{ $products->name }}</h2>
        <p class="text-xl py-3">IDR {{ number_format($products->price)}}</p>
      </div>
      <form action="{{ route('cart-add', $products->id) }}" method="POST">
        @csrf
        <button type="submit"
          class="transition-all duration-200 bg-pink-400 text-black focus:bg-black focus:text-pink-400 rounded-full px-8 py-3 mt-4 inline-flex w-full md:w-auto text-center justify-center">
        ...
        <div class="rounded-xl p-4 pb-8 relative bg-white">
          <div class="rounded-xl overflow-hidden card-shadow w-full h-36">
            <img
              ...
              alt="{{ $recomendation->name }}" class="w-full h-full object-cover object-center" />
          </div>
