<x-app-layout>
  <x-slot name="header">
    <h2 class="font-semibold text-xl text-gray-800 leading-tight">
      {{ $item->roles }} &raquo; {{$item->name }} &raquo; Edit
    </h2>
  </x-slot>
      ...
      <form action="{{ route('dashboard.user.update',$item->id) }}" class="w-full " method="post"
        ...
        <div class="flex flex-wrap -mx-3 mb-6 ">
          <div class="w-full px-3">
            ...
            <input type="text" placeholder="User Name" value="{{ old('name') ?? $item->name }}" name="name"
        ...
        <div class="flex flex-wrap -mx-3 mb-6 ">
          <div class="w-full px-3">
            <label for="" class="block uppercase tracking-white text-gray-700 text-xs font-bold mb-2">Email</label>
            <input type="text" placeholder="User Email" value="{{ old('email') ?? $item->email }}" name="email"
              class="block w-full bg-gray-200 text-gray-700 border boder-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
          </div>
        </div>
        <div class="flex flex-wrap -mx-3 mb-6 ">
          <div class="w-full px-3 ">
            <label for="" class="block uppercase tracking-white text-gray-700 text-xs font-bold mb-2">Roles</label>
            <select name="roles"
              class="block w-full bg-gray-200 text-gray-700 border boder-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
